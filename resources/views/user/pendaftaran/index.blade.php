...
<div class="container">
    ...
    <a href="{{ route('user.pendaftaran.create') }}" class="btn btn-primary mb-3">Tambah Pendaftaran</a>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $pend->status }}</td>
                <td>
                    <a href="{{ route('user.pendaftaran.edit', $pend->id) }}" class="btn btn-warning btn-sm">Edit</a>
                    <form action="{{ route('user.pendaftaran.destroy', $pend->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
